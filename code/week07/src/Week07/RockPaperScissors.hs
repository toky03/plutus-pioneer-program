...
data GameRedeemer = Play GameChoice | Reveal ByteString GameChoice | ClaimFirst | ClaimSecond deriving (Show)
...
transition game state redeemer = case (stateValue state, stateData state, redeemer) of
  (v, GameDatum bs Nothing, Play choice) -- player two wants to make a game
    | lovelaces v == gStake game ->
      Just
        ( Constraints.mustBeSignedBy (gSecond game)
            <> Constraints.mustValidateIn (to $ gPlayDeadline game),
          State (GameDatum bs $ Just choice) (lovelaceValueOf $ 2 * gStake game) -- first players stake plus second players stake
        )
  (v, GameDatum _ (Just c), Reveal _ c')
    | lovelaces v == (2 * gStake game) && c `beats` c' ->
      Just
        ( Constraints.mustBeSignedBy (gFirst game)
            <> Constraints.mustValidateIn (to $ gRevealDeadline game),
          State Finished mempty
        )
    | lovelaces v == (2 * gStake game) && c' == c ->
      Just
        ( Constraints.mustBeSignedBy (gFirst game) <> Constraints.mustValidateIn (to $ gRevealDeadline game) <> Constraints.mustPayToPubKey (gSecond game) (lovelaceValueOf $ gStake game),
          State Finished mempty
        )
...
-- the nonce check cannot be done with TxConstraint violations
-- therefore we have to specify this in the check function
...
check bsRock' bsPaper' bsScissors' (GameDatum bs (Just _)) (Reveal nonce choice) _ =
  sha2_256 (nonce `concatenate` toBS choice) == bs
  where
